...
using Pally.Application.Features.VerifyIdentity.Commamds;
using PhoneNumbers;
...
namespace Pally.Application.Common
{
    public static class CommonHelper
    {
        ...
        public static bool IsValidEmail(string email)
        {
            if (string.IsNullOrEmpty(email))
                return false;
            email = email.Trim();
            var result = Regex.IsMatch(email, "^(?:[\\w\\!\\#\\$\\%\\&\\'\\*\\+\\-\\/\\=\\?\\^\\`\\{\\|\\}\\~]+\\.)*[\\w\\!\\#\\$\\%\\&\\'\\*\\+\\-\\/\\=\\?\\^\\`\\{\\|\\}\\~]+@(?:(?:(?:[a-zA-Z0-9](?:[a-zA-Z0-9\\-](?!\\.)){0,61}[a-zA-Z0-9]?\\.)+[a-zA-Z0-9](?:[a-zA-Z0-9\\-](?!$)){0,61}[a-zA-Z0-9]?)|(?:\\[(?:(?:[01]?\\d{1,2}|2[0-4]\\d|25[0-5])\\.){3}(?:[01]?\\d{1,2}|2[0-4]\\d|25[0-5])\\]))$", RegexOptions.IgnoreCase, Timeout);
            return result;
        }

        public static bool validateNIN(string value)
        {
            if (string.IsNullOrEmpty(value))
                return false;
            value = value.Trim();
            var result = Regex.IsMatch(value, "^[0-9]{11}$", RegexOptions.IgnoreCase, Timeout);
            return result;
        }
...
;
        ...
        public static string decryptPayload(DateTime datetime, string applicationId, [FromBody] string encryptedpayload)
        {
            ...
            string key = Convert.ToBase64String(Encoding.UTF8.GetBytes(formattedOmtid)).Substring(0, 16);
            var iv = Guid.Parse(applicationId).ToString("N").Substring(0, 16);
            var decryptedData = CommonHelper.DecryptStringFromAES(encryptedpayload, key, iv);
            return decryptedData;
        }
